document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Audio Pre-rendering (Fast AudioBuffer playback) ---
    let audioMode = 'prerendered'; // 'original' or 'prerendered'
    const preRenderedSounds = { tink: null, tonk: null };
    const PRERENDER_SCALE = 0.25; // Scale down safely during rendering to avoid clipping
    ...
    async function preRenderAudio() {
        ...
        // Render TINK
        try {
            const tinkCtx = new OfflineCtx(1, sampleRate * 0.1, sampleRate);
            const osc = tinkCtx.createOscillator();
            const gain = tinkCtx.createGain();
            osc.type = 'sine';
            osc.frequency.setValueAtTime(1500, 0);
            osc.frequency.exponentialRampToValueAtTime(800, 0.1);
            gain.gain.setValueAtTime(1.5 * PRERENDER_SCALE, 0);
            gain.gain.exponentialRampToValueAtTime(0.001, 0.1);
            
            const noise = tinkCtx.createBufferSource();
            noise.buffer = noiseBuffer;
            const noiseFilter = tinkCtx.createBiquadFilter();
            noiseFilter.type = 'highpass';
            noiseFilter.frequency.value = 2000;
            const noiseGain = tinkCtx.createGain();
            noiseGain.gain.setValueAtTime(0.5 * PRERENDER_SCALE, 0);
            noiseGain.gain.exponentialRampToValueAtTime(0.001, 0.05);
            
            noise.connect(noiseFilter);
            noiseFilter.connect(noiseGain);
            noiseGain.connect(tinkCtx.destination);
            osc.connect(gain);
            gain.connect(tinkCtx.destination);
            
            osc.start(0); osc.stop(0.1);
            noise.start(0); noise.stop(0.05);
            
            preRenderedSounds.tink = await tinkCtx.startRendering();
        } catch (e) { console.error("Prerender Tink failed:", e); }

        // Render TONK
        try {
            const tonkCtx = new OfflineCtx(1, sampleRate * 0.2, sampleRate);
            const osc = tonkCtx.createOscillator();
            const gain = tonkCtx.createGain();
            osc.type = 'triangle';
            osc.frequency.setValueAtTime(450, 0);
            osc.frequency.exponentialRampToValueAtTime(200, 0.2);
            gain.gain.setValueAtTime(2.0 * PRERENDER_SCALE, 0);
            gain.gain.exponentialRampToValueAtTime(0.001, 0.2);
            
            const filter = tonkCtx.createBiquadFilter();
            filter.type = 'bandpass';
            filter.frequency.value = 600;
            
            const noise = tonkCtx.createBufferSource();
            noise.buffer = noiseBuffer;
            const noiseFilter = tonkCtx.createBiquadFilter();
            noiseFilter.type = 'bandpass';
            noiseFilter.frequency.value = 800;
            const noiseGain = tonkCtx.createGain();
            noiseGain.gain.setValueAtTime(1.0 * PRERENDER_SCALE, 0);
            noiseGain.gain.exponentialRampToValueAtTime(0.001, 0.05);
            
            noise.connect(noiseFilter);
            noiseFilter.connect(noiseGain);
            noiseGain.connect(tonkCtx.destination);
            
            osc.connect(filter);
            filter.connect(gain);
            gain.connect(tonkCtx.destination);
            
            osc.start(0); osc.stop(0.2);
            noise.start(0); noise.stop(0.05);
            
            preRenderedSounds.tonk = await tonkCtx.startRendering();
        } catch (e) { console.error("Prerender Tonk failed:", e); }
    }

    // Call pre-render immediately on load
    preRenderAudio();
    ...
    function playBuffer(buffer) {
        ...
        // Restore volume scaling that was reduced to prevent clipping in prerender
        const gainNode = audioContext.createGain();
        gainNode.gain.value = finalVolume() / PRERENDER_SCALE; 
    ...
    function playTink() {
        if (audioMode === 'prerendered' && preRenderedSounds.tink) {
            playBuffer(preRenderedSounds.tink);
        } else {
            playTinkOriginal();
        }
    }
    ...
    function playTonk() {
        if (audioMode === 'prerendered' && preRenderedSounds.tonk) {
            playBuffer(preRenderedSounds.tonk);
        } else {
            playTonkOriginal();
        }
    }

    function playTinkOriginal() {
